...
scientific: true
---
On the 25th of June this year the new IDCC(c) capacity calculation will go live. This is the third of in total five planned capacity calculations in the intraday timeframe. For each of these the capacities will be recalculated with updated forecasts and taking into account the trades up until the point of (re)calculation.  
In this post an overview of the intraday process for the Core region is presented. After that the results of the parallel run of the new (c) calculation will be discussed. These results show that a "reflow" effect is ocurring, in which available capacities are redistributed around the current market clearing point, opening isolated zones and borders, in exchange for a lower average positive capacity. Market parties can study these results to see what they can expect after go-live.

# The process and its timings
The intraday market of SIDC is based on both auctions and continious trade. Currently the _capacity allocation_ for both of these is still done in ATC, while the _capacity calculation_ is done in the flowbased methodology. This is because the research and development on flowbased _allocation_ in intraday is still ongoing and it is thus currently not yet possible. In Day-ahead, allocation is already done in flowbased.  
...
IDCC(a) is a little different. This is the calculation directly after day-ahead market clearing and is not a full new calculation. Instead it simply extracts the leftovers from the day-ahead calculation after the allocated trades from the day-ahead auction are subtraced.  
...
In order to properly assess the effect of the IDCC(c) calculation we want to compare the ATC's just before and after IDCC(c). Simply comparing with IDCC(b) results would include trade in between (b) and (c). By calculating the ATC just _before_ IDCC(c) and compare it with the output _after_ (c) this effected is removed. This then results in the formula:
...
To analyse the results of the parallel run three indicators will be discussed next. In these the discussed delta is applied to be able to give a fair comparison.
...
This means that if a border had a very high ATC available but a different border is negative before the calculation, the extraction algorithm will redistribute the available capacity fairly among them (if the capacity still exists after updating all inputs). If a border sees a lot of trade, resulting in low left over ATC, the same thing will occur. This effect can be seen as a "reflow" of available capacities across all borders, resulting in the indicators we are seeing.  


# What is next
After the go live of IDCC(c) TSO's will work on developing and bringing live the remaining planned flowbased calculations of (d) and (e). It is exptected that for the hours they calculate (12:00 onwards for (d) and 19:00 onwards for (e)) the same reflow effect will occur as is happening in (c). For these a paralell run will be held by the TSO's as well, hopefully proving the same positive results.
